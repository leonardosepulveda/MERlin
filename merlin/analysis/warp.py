...
class FiducialCorrelationWarp(Warp):
    ...
    def _load_drift_dataframe(self) -> pd.DataFrame:
        """Per-(fov, data channel) fiducial registration shift, in pixels,
        read back from the transformations this task already saved.
        """
        dataChannels = self.dataSet.get_data_organization().get_data_channels()
        rows = [{'fov': fov, 'dataChannel': dc,
                ...
                for dc, t in zip(dataChannels, self.get_transformation(fov))]
    ...
    def _generate_verification_figures(self) -> None:
        ...
        """
        driftDF = self._load_drift_dataframe()
        if driftDF.empty:
            return
        ...
        fig.suptitle('%s: fiducial registration shift QC' % self.analysisName)
        ...
        self.dataSet.save_task_figure(self, fig, 'drift_qc')
        plt.close(fig)
